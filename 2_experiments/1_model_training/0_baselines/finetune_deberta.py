...
def finetune(model_identifier, conf):
    train_data = pd.read_csv('data/dso_dataset_translated/rewire_dso_2022-12-05-1404_train_english.csv', index_col=0)
    valid_data = pd.read_csv('data/dso_dataset_translated/rewire_dso_2022-12-05-1404_val_english.csv', index_col=0)
    test_data = pd.read_csv('data/dso_dataset_translated/rewire_dso_2022-12-05-1404_test_english.csv', index_col=0)
    ...
    ds = (Dataset.from_pandas(train_data)).map(tokenize_function).remove_columns(["rewire_id", "text"])
    preds = evaluator.predict(ds).predictions
    probabilities = nn.functional.softmax(from_numpy(preds), dim=-1)
    train_data[model_name] = probabilities[:, 1]
    train_data.to_csv('outputs/evaluation/' + model_name + "/train_set_predictions.csv")
